...
function mgfit_synth_spec_ut::test_basic
  compile_opt strictarr
  
  spectrumstructure={wavelength: 0.0, flux:0.0, residual:0.0}
  
  base_dir = file_dirname(file_dirname((routine_info('mgfit_synth_spec_ut__define', /source)).path))
  data_dir = ['data']
  ...
  temp=size(specsynth,/DIMENSIONS)
  result=temp[0]

  assert, result eq 40, 'incorrect result: %d', result
  
  return, 1
end

pro mgfit_synth_spec_ut__define
  compile_opt strictarr
  
  define = { mgfit_synth_spec_ut, inherits mgfitUTTestCase}
end
