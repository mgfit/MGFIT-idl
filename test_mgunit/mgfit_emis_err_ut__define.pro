...
function mgfit_emis_err_ut::test_basic
  compile_opt strictarr
  
  spectrumstructure={wavelength: 0.0, flux:0.0, residual:0.0}
  
  base_dir = file_dirname(file_dirname((routine_info('mgfit_emis_err_ut__define', /source)).path))
  data_dir = ['data']
  ...
  specsynth[*].wavelength=spectrumdata.wavelength
  specsynth[*].flux=0.0
  specsynth=mgfit_synth_spec(emissionlines_section, specsynth)
  emissionlines_section=mgfit_emis_err(specsynth, spectrumdata, emissionlines_section, redshift_initial)
...
pro mgfit_emis_err_ut__define
  compile_opt strictarr
  
  define = { mgfit_emis_err_ut, inherits mgfitUTTestCase}
end
